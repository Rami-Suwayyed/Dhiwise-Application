...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Estella30' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'RfzbpI6bgyTFd2D',
        'isDeleted':false,
        'username':'Estella30',
        'email':'Chelsie_Torp@gmail.com',
        'isActive':true,
      ...
      await dbService.update(model.user, { 'username':'Estella30' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'Arnulfo40' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'flqyeGtnHtpfCYS',
        'isDeleted':false,
        'username':'Arnulfo40',
        'email':'Deron_Nicolas@gmail.com',
        'isActive':true,
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.update(model.user, { 'username':'Arnulfo40' }, userToBeInserted);
    }
